...
function Logo({ className }: { className?: string }) {
  return (
    <div className={cn("flex items-center gap-2.5", className)}>
      <div
        className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg font-mono text-[13px] font-bold text-white shadow-md"
...
// Painel "court" — gradiente + grid sutil no lugar de uma foto.
function CourtVisual({ children }: { children?: ReactNode }) {
  return (
    <div
    ...
    >
      ...
      <div
        ...
        aria-hidden
      />
      {/* linha central e círculo da quadra */}
      <div className="absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-border" aria-hidden />
      <div
        className="absolute left-1/2 top-1/2 h-24 w-24 -translate-x-1/2 -translate-y-1/2 rounded-full border border-border"
        aria-hidden
      />
      {children}
    </div>
...
const FEATURES: Feature[] = [
  {
    ...
    desc: "Probabilidade real estimada a partir dos últimos 10 jogos, ponderada por playoffs, defesa e pace do adversário — comparada à odd da casa.",
  },
  {
    icon: <IconClock />,
    title: "Odds ao vivo",
  ...
  },
  {
    ...
    desc: "Cada entrada classificada como STRONG, VALUE, NEUTRAL ou AVOID — critérios transparentes de EV mínimo e probabilidade real.",
  },
  {
    icon: <IconTrophy />,
  ...
  },
  {
    ...
    desc: "Resultados salvos entre sessões e quota da Odds API monitorada — 500 requests/mês rendem a temporada inteira.",
  },
];
    ...
    <div className="min-h-screen bg-canvas font-sans text-fg">
      ...
      <section className="relative overflow-hidden">
        <div
          ...
          style={{ background: "radial-gradient(60% 100% at 50% 0%, rgba(99,102,241,0.16), transparent)" }}
          aria-hidden
        />
        <m.div
          variants={container}
        ...
        >
          ...
          <m.div variants={item} className="mt-9 flex flex-wrap items-center justify-center gap-3">
            <button
              onClick={openPanel}
              className="inline-flex items-center gap-2 rounded-xl bg-fg px-6 py-3 text-base font-semibold text-canvas transition-transform hover:scale-[1.02] active:scale-[.98]"
            >
      ...
      <section id="recursos" className="scroll-mt-20 border-t border-border bg-surface/40">
        <div className="mx-auto max-w-[1200px] px-5 py-20 md:px-8 md:py-28">
          <div className="mb-14 text-center">
            <div className="mb-3 font-mono text-xs tracking-[0.28em] text-fg-subtle uppercase">Recursos</div>
          ...
          <m.div variants={container} initial="hidden" animate="show" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((f) => (
              ...
              >
                <div className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-raised text-fg-muted">
                  {f.icon}
                </div>
        ...
        <CourtVisual>
          <div className="absolute inset-0 flex flex-col items-center justify-center px-5 text-center">
            ...
            <div className="relative">
              ...
              <button
                onClick={openPanel}
                className="mt-8 inline-flex items-center gap-2 rounded-xl bg-fg px-6 py-3 text-base font-semibold text-canvas transition-transform hover:scale-[1.02] active:scale-[.98]"
              >
                <span aria-hidden>⊞</span> Abrir o painel
              </button>
            </div>
