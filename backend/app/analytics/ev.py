...
_AVG_KEY: dict[str, str] = {
    ...
    "FG3M": "avg_3pm",  "BLK": "avg_blk",  "STL": "avg_stl",
    "PRA": "avg_pra",   "PR": "avg_pr",    "PA": "avg_pa",
    "RA": "avg_ra",     "STOCKS": "avg_stocks",
}

_STD_KEY: dict[str, str] = {
    "PTS": "std_pts",   "REB": "std_reb",  "AST": "std_ast",
    "FG3M": "std_3pm",  "BLK": "std_blk",  "STL": "std_stl",
    "PRA": "std_pra",   "PR": "std_pr",    "PA": "std_pa",
    "RA": "std_ra",     "STOCKS": "std_stocks",
}
...
def _ratio_to_prob(ratio: float) -> float:
    ...
    if ratio >= 1.10: return 0.68
    if ratio >= 1.00: return 0.58
    if ratio >= 0.90: return 0.45
    if ratio >= 0.80: return 0.37
    return 0.28
...
def estimate_true_probability(
    ...
    if season_avg > 0 and line > 0:
        season_prob = (_norm_sf(line, season_avg, std_stat_orig)
                       if std_stat_orig > 0 else _ratio_to_prob(season_avg / line))
    else:
    ...
    if (is_playoffs and playoff_history
            and playoff_history.get("games", 0) >= _PLAYOFF_HIST_MIN_GAMES):
        hist_avg = playoff_history.get(avg_key, 0.0)
        if hist_avg > 0 and line > 0:
            hist_p = (_norm_sf(line, hist_avg, std_stat_orig)
                      if std_stat_orig > 0 else _ratio_to_prob(hist_avg / line))
    ...
    if   def_rating >= L + 4: def_scale = 1.05
    elif def_rating >= L:     def_scale = 1.02
    elif def_rating <= L - 6: def_scale = 0.94
    elif def_rating <= L - 2: def_scale = 0.97
    else:                     def_scale = 1.0

    if   pace > 105: pace_scale = 1.03
    elif pace > 102: pace_scale = 1.02
    elif pace < 96:  pace_scale = 0.98
    else:            pace_scale = 1.0
    ...
    if (def_scale != 1.0 or pace_scale != 1.0) and std_stat > 0 and avg_stat > 0 and line > 0:
        ...
        if   def_rating >= L + 4: p_over += 0.04
        elif def_rating >= L:     p_over += 0.02
        elif def_rating <= L - 6: p_over -= 0.05
        elif def_rating <= L - 2: p_over -= 0.03

        if   pace > 105: p_over += 0.03
        elif pace > 102: p_over += 0.02
        elif pace < 96:  p_over -= 0.02
